...
! Infinite Ammo Storage:
  - No need to for crates anymore.
  - Still need reloading.
  - Should not be used on leaderboards, this is not moral.
! Bottomless Magazine:
  - Glitchy when used with burst-fire weapons, causing "infinite autofire".
  - Swap weapon to fix the glitch.
  - Yeah, don't try this on leaderboards, either.
! No Perk Cooldown:
  - Literally, no, don't, this is cheating...
]]
    )
end
...
function Main()
    ...
    if MN ~= nil then
        ...
        if xorBool(MN[7], switch[7]) then
            switch[7] = not switch[7]
            infAmmo(MN[7])
        end
        if xorBool(MN[8], switch[8]) then
            switch[8] = not switch[8]
            superMag(MN[8])
        end
        if xorBool(MN[9], switch[9]) then
            switch[9] = not switch[9]
            infPerk(MN[9])
        end
    end
    gg_window_status = -1
...
function infAmmo(toggleon)
    local org = 'h FE 4F BF A9 08 14 40 F9 88 02 00 B4 F3 03 00 AA 00 09 40 F9 20 02 00 B4 E1 03 1F AA 71 11 03 94 60 00 00 36 20 00 80 52 0A 00 00 14 68 16 40 F9 48 01 00 B4 08 1D 40 F9 08 01 00 B4 08 31 40 F9'
    local mod = 'h 20 00 80 52 C0 03 5F D6 88 02 00 B4 F3 03 00 AA 00 09 40 F9 20 02 00 B4 E1 03 1F AA 71 11 03 94 60 00 00 36 20 00 80 52 0A 00 00 14 68 16 40 F9 48 01 00 B4 08 1D 40 F9 08 01 00 B4 08 31 40 F9'
    local byte_length = 64
    --[[
    bool __fastcall AmmoComponent__get_UnlimitedAmmo
    MOV W0, #1
    RET  
    20 00 80 52 C0 03 5F D6
    --]]
    ...
    if toggleon then
        safeAoBReplace(org, mod, byte_length)
        gg.toast("Activated: > Infinite Ammo")
    else
        safeAoBReplace(mod, org, byte_length)
        gg.toast("Deactivated: > Infinite Ammo")
    end
end

function superMag(toggleon)
    local org = 'h FE 4F BF A9 08 14 40 F9 88 02 00 B4 F3 03 00 AA 00 09 40 F9 20 02 00 B4 E1 03 1F AA 5A 11 03 94 60 00 00 36 20 00 80 52 0A 00 00 14 68 16 40 F9 48 01 00 B4 08 1D 40 F9 08 01 00 B4 08 31 40 F9'
    local mod = 'h 20 00 80 52 C0 03 5F D6 88 02 00 B4 F3 03 00 AA 00 09 40 F9 20 02 00 B4 E1 03 1F AA 5A 11 03 94 60 00 00 36 20 00 80 52 0A 00 00 14 68 16 40 F9 48 01 00 B4 08 1D 40 F9 08 01 00 B4 08 31 40 F9'
    local byte_length = 64
    --[[
    bool __fastcall AmmoComponent__get_UnlimitedClip
    MOV W0, #1
    RET  
    20 00 80 52 C0 03 5F D6
    --]]
...
function infPerk(toggleon)
    local org = 'h E9 23 BE 6D FE 4F 01 A9 F3 03 00 AA E0 03 1F AA 3B 7A F1 97 60 04 00 B4 08 04 41 F9 28 04 00 B4 08 C1 44 39 68 00 00 34 E0 03 1F 2A 1A 00 00 14 60 26 40 BD 08 20 20 1E 25 01 00 54 E0 03 1F AA'
    local mod = 'h 20 00 80 52 C0 03 5F D6 F3 03 00 AA E0 03 1F AA 3B 7A F1 97 60 04 00 B4 08 04 41 F9 28 04 00 B4 08 C1 44 39 68 00 00 34 E0 03 1F 2A 1A 00 00 14 60 26 40 BD 08 20 20 1E 25 01 00 54 E0 03 1F AA'
    local byteLength = 64
    --[[
    bool __fastcall SquadAbility__get_IsReady
    MOV W0, #1
    RET  
    20 00 80 52 C0 03 5F D6
    --]]
    ...
    if toggleon then
        safeAoBReplace(org, mod, byteLength)
        gg.toast("Activated: > No Perk Cooldown")
    else
        safeAoBReplace(mod, org, byteLength)
        gg.toast("Deactivated: > No Perk Cooldown")
    end
end
